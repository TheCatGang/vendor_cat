...
if __name__ == '__main__':
    # Default to CAT Gerrit
    default_gerrit = 'https://review.catgzr.com'
    ...
    for item in mergables:
        ...
        if args.gerrit == default_gerrit:
            ...
            if args.pull:
                cmd = ['git pull --no-edit cat', item['fetch'][method]['ref']]
            else:
                cmd = ['git fetch cat', item['fetch'][method]['ref']]
            if args.quiet:
                cmd.append('--quiet')
        ...
        if args.gerrit != default_gerrit or os.stat(FETCH_HEAD).st_size == 0:
            # If not using the default gerrit or cat failed, fetch from gerrit.
            if args.verbose:
                if args.gerrit == default_gerrit:
