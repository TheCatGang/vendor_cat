...
func init() {
	android.RegisterModuleType("cat_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = catExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = catExpandVariables(ctx, dep_file)
	...
	cmd := catExpandVariables(ctx, String(g.properties.Cmd))
